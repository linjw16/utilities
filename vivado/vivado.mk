...
###################################################################
path_ip = ./fpga.srcs/sources_1/ip/
name_ip = eth_xcvr_gt
...
clean:: tmpclean
	-rm -rf *.bit program.tcl generate_mcs.tcl *.mcs *.prm flash.tcl pre_*tcl
distclean:: clean
	-rm -rf rev
...
# synthesis run
pre_synth.tcl:  $(FPGA_TOP).xpr
	echo "open_project $(FPGA_TOP).xpr" > $@
	echo "reset_run synth_1" >> $@
	echo "launch_runs -jobs 32 synth_1" >> $@
#	echo "wait_on_run synth_1" >> $@
	vivado -nojournal -nolog -mode batch -source $@

pre_work.tcl: pre_synth.tcl
	/bin/cp -rf .fpga.gen/* fpga.gen/
	echo "open_project -quiet $(FPGA_TOP).xpr" > $@
	echo "set_property generate_synth_checkpoint false [get_files $(path_ip)/$(name_ip)_1/$(name_ip)_1.xci]" >> $@
	echo "set_property generate_synth_checkpoint false [get_files $(path_ip)/$(name_ip)_3/$(name_ip)_3.xci]" >> $@
	echo "set_property IS_MANAGED 0  [get_files $(path_ip)/$(name_ip)_1/$(name_ip)_1.xci]" >> $@
	echo "set_property IS_MANAGED 0  [get_files $(path_ip)/$(name_ip)_3/$(name_ip)_3.xci]" >> $@
	vivado -nojournal -nolog -mode batch -source $@

%.runs/synth_1/%.dcp: pre_work.tcl $(SYN_FILES_REL) $(INC_FILES_REL) $(XDC_FILES_REL)
